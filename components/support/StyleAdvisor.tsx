            ...
            {[1, 2, 3].map(i => (
                <div key={i} className="hidden md:flex h-6 w-6 rounded-full border-2 border-orange-500 bg-orange-400 items-center justify-center text-[10px] font-black italic relative overflow-hidden">
                    <Image 
                        ...
                        fill
                        unoptimized
                        className="opacity-80 grayscale group-hover:grayscale-0 transition-all object-cover" 
                    />
                </div>
            ))}
        </div>
...
function AdvisorModal({ onClose }: { onClose: () => void }) {
  ...
  const [messages, setMessages] = useState<AdvisorMessage[]>([
        { 
        role: 'assistant', 
        content: "Hi! I'm your Stond AI Advisor. I'm ready to help you find your next signature items. Should I start by analyzing your recent orders to see what you like?",
        actions: [
            ...
            { label: "Discover something new", value: "build_outfit", icon: <Zap size={14} /> }
        ]
    }
  ]);
        ...
        if (input === "analyze_history") {
            ...
            if (result.recommendations && result.recommendations.length > 0) {
                 setTimeout(() => {
                    setMessages(prev => [...prev, { 
                        role: 'assistant', 
                        content: "Here are some actual items from our catalog that align with your profile:",
      ...
      >
        ...
        <div className="p-6 md:p-8 border-b border-white/5 flex items-center justify-between bg-gradient-to-b from-white/[0.02] to-transparent">
          <div className="flex items-center gap-4">
            ...
            <div>
                <h3 className="text-xl font-black text-white tracking-tight">AI Advisor</h3>
            ...
            >
                ...
                <div className="flex-1 max-w-[85%] space-y-4">
                    <div className={cn(
                        ...
                        {msg.image && (
                            <div className="mb-3 rounded-xl overflow-hidden border border-white/10 shadow-lg relative h-[200px] w-full">
                                <Image 
                                    src={msg.image} 
                                    alt="User upload" 
                                    fill
                    ...
                    {msg.actions && (
                        <div className="flex flex-wrap gap-2">
                            {msg.actions.map((action: AdvisorAction) => (
                                <button
                                    key={action.value}
                            ...
                            <div className="w-full p-4 bg-white/[0.02] border border-white/5 rounded-[2rem] overflow-hidden group/product transition-all hover:bg-white/5">
                                ...
                                <div className="flex items-center justify-between gap-4 px-2">
                                    ...
                                    <div className="flex gap-2 shrink-0">
                                        ...
                                        <button 
                                            onClick={() => { if (msg.product?.slug) window.location.href = `/products/${msg.product.slug}` }}
                    ...
                    {msg.products && (
                        <div className="space-y-4 w-full">
                            <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide -mx-1">
                                {msg.products.map((product: RecommendationItem) => (
          ...
          <form onSubmit={handleSend} className="relative group">
            ...
            {imagePreview && (
                <div className="absolute bottom-full left-0 mb-4 p-2 bg-[#1a1a1a] border border-white/10 rounded-2xl flex items-center gap-3 animate-in fade-in slide-in-from-bottom-2 shadow-2xl">
                    <div className="h-16 w-16 rounded-xl overflow-hidden border border-white/20 relative">
            ...
            <div className="absolute left-2 top-1/2 -translate-y-1/2">
                <button 
                    ...
                    className="h-10 w-10 bg-white/5 hover:bg-white/10 rounded-full flex items-center justify-center text-white/60 hover:text-white transition-all group/cam"
                    title="Upload photo"
                >
            ...
            <input 
                ...
                className="w-full h-14 pl-14 pr-16 bg-black border border-white/10 rounded-full text-white outline-none focus:border-orange-500/50 transition-all font-medium text-base placeholder:text-white/40"
            />
            <button 
                type="submit"
                disabled={(!inputValue.trim() && !selectedImage) || isTyping}
                className="absolute right-2 top-1/2 -translate-y-1/2 h-10 w-10 bg-orange-500 hover:bg-orange-600 disabled:opacity-20 rounded-full flex items-center justify-center text-white transition-all active:scale-95 shadow-lg shadow-orange-500/20"
            >
                <Send size={16} />
            </button>
          </form>
          <div className="mt-4 flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {["Analyze my taste", "Monochrome looks", "Boho summer", "Business casual"].map(tag => (
                <button 
                    ...
                    className="px-4 py-2 rounded-xl bg-white/[0.05] border border-white/5 text-[10px] font-black uppercase tracking-widest text-white hover:bg-orange-500/20 hover:text-orange-500 transition-all whitespace-nowrap"
                >
                    {tag}
                </button>
            ))}
