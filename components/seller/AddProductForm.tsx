...
export function AddProductForm({ categories }: AddProductFormProps) {
    ...
    if (saved) {
      try {
        ...
        setFormData(prev => ({
          ...prev,
          ...parsed,
          // Special handling for media to ensure we don't store/load stale URLs if they expire
          // But IDs are fine. For now, we'll keep it simple.
        }));
      } catch (e) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        // Filter out incomplete variants - only include variants that have all required fields
                                    ...
                                    onChange={async (e) => {
                                        ...
                                        try {
                                            ...
                                            for (const file of Array.from(files)) {
                                                ...
                                                console.log("Uploaded media item:", { id: data.media.id, url: data.media.url });
                                            }
                    ...
                    <CardContent className="p-10 space-y-10">
                        <div className="grid gap-8 md:grid-cols-2">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="h-14 px-6 flex items-center justify-between bg-slate-50 rounded-2xl border border-slate-100 opacity-60">
                                    <div className="flex items-center gap-3">
                                        <div className="h-2 w-2 rounded-full bg-amber-500 animate-pulse" />
                                        <span className="font-black text-[10px] uppercase tracking-[0.2em]">DRAFT</span>
                                    </div>
                                    <div className="flex items-center gap-2 bg-indigo-50 text-indigo-600 px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest border border-indigo-100">
                                        <Zap className="h-2.5 w-2.5" /> Auto-Queued
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <Card className="border-none shadow-[0_-20px_50px_rgba(0,0,0,0.1)] bg-slate-900 rounded-[2rem] overflow-hidden border border-white/10">
                    <CardContent className="p-6 flex items-center justify-between gap-6">
                        <div className="hidden md:block">
                            <p className="text-white font-black text-sm uppercase tracking-widest pl-4">Draft Saved Locally</p>
                            <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest pl-4">Last update: Just now</p>
                        </div>
                        <div className="flex items-center gap-4 w-full md:w-auto">
                            ...
                            >
                                {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><Save className="h-5 w-5" /> Launch Product</>}
                            </Button>
                        </div>
                    </CardContent>
                </Card>
