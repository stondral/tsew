...
export async function acceptOrderAction(
  ...
  const { user } = await payload.auth({ headers: requestHeaders }) as { user: User | null };
...
export async function searchOrderAction(query: string) {
  ...
  try {
    ...
    if (result.docs.length > 0) {
      return { ok: true, id: result.docs[0].id };
    }
...
export async function updateOrderAddressAction(
  ...
  try {
    ...
    // Update the embedded shipping address on the order
