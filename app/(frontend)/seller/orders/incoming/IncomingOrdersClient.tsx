...
import { Package, Truck, Check, X, AlertCircle, Sparkles, PartyPopper } from "lucide-react"
...
export default function IncomingOrdersClient({ orders }: { orders: any[] }) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function DeliveryModal({ order, onClose, onSuccess }: { order: any, onClose: () => void, onSuccess: () => void }) {
  ...
  const handleAccept = async () => {
    ...
    // We are already in an event handler, we don't strictly need startTransition for the fetch itself,
    // but we use it to wrap the state updates that follow.
    startTransition(() => {
       (async () => {
  ...
  const gst = selectedProvider ? selectedProvider.cost * 0.18 : 0
  const totalWithGst = selectedProvider ? selectedProvider.cost + gst : 0
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      <motion.div 
        ...
        className="relative bg-white rounded-[32px] w-full max-w-md overflow-hidden shadow-2xl border border-white"
      >
        <div className="p-8">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
          ...
          <p className="text-sm text-slate-500 mb-6 font-medium">Select a delivery partner to fulfill this order. Costs include 18% GST.</p>
              ...
              <button
                ...
                className={`w-full flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${
                  selectedProvider?.name === p.name
                    ...
                    : "border-slate-100 hover:border-amber-200 bg-white shadow-sm"
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`h-5 w-5 rounded-full border-2 flex items-center justify-center transition-all ${
                    selectedProvider?.name === p.name ? "border-amber-500 bg-amber-500" : "border-slate-200"
                  }`}>
                    {selectedProvider?.name === p.name && <Check className="h-3 w-3 text-white stroke-[4]" />}
                  </div>
                  <span className={`font-bold ${selectedProvider?.name === p.name ? "text-slate-900" : "text-slate-600"}`}>{p.name}</span>
                </div>
          ...
          {selectedProvider && (
            <motion.div 
              ...
              className="mt-8 p-6 bg-slate-50 rounded-3xl border border-slate-100 space-y-2 overflow-hidden"
            >
              ...
              <div className="flex justify-between text-lg font-black text-slate-900">
                ...
                <span className="text-amber-500">₹{totalWithGst.toFixed(2)}</span>
              </div>
            </motion.div>
          )}
          ...
          {error && (
            <motion.div 
              ...
              className="mt-4 p-4 bg-red-50 text-red-600 rounded-2xl flex items-center gap-3 text-sm font-bold border border-red-100 italic"
            >
          ...
          <div className="grid grid-cols-2 gap-3 mt-8">
            ...
            <Button 
              ...
              className="bg-slate-900 hover:bg-black text-white font-bold rounded-2xl h-14 shadow-lg transition-all active:scale-95 disabled:bg-slate-200"
            >
              {isPending ? "Processing..." : (
                <span className="flex items-center gap-2">
                  Confirm & Accept <PartyPopper className="h-4 w-4" />
                </span>
              )}
            </Button>
          </div>
