...
export default async function WarehousesPage() {
  ...
  if (!user || ((user as User).role !== "seller" && (user as User).role !== "admin" && (user as User).role !== "sellerEmployee")) {
    redirect("/auth?redirect=/seller/warehouses");
  }

  // Get sellers where user has warehouse.view permission
  const { getSellersWithPermission } = await import('@/lib/rbac/permissions');
  const allowedSellers = await getSellersWithPermission(payload, user.id, 'warehouse.view');
