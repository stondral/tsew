...
export default async function AddProductPage() {
  ...
  if (!user || (userRole !== "seller" && userRole !== "admin" && userRole !== "sellerEmployee")) {
    redirect("/auth?redirect=/seller/products/add");
  }

  const { isUserPartOfActiveSeller } = await import('@/lib/rbac/permissions');
  ...
  // Subscription Check - check if user belongs to ANY active seller organization
  const isActive = (userRole === 'admin') || await isUserPartOfActiveSeller(payload, user.id);
